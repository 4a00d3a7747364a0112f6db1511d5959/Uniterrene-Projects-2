...
class Oscprofessionals_Flatship_Model_Carrier_Oscpflatshiprate
    ...
    {    
      ...
      if(!Mage::helper('oscpflatshiprate')->isEnable()){
        ...
        try {
			 //echo $sub_total = Mage::getSingleton('checkout/session')->getQuote()->getSubtotal();
            $shippingPrice = '';
           ...
           // echo Mage::helper('checkout')->formatPrice(Mage::getModel('checkout/cart')->getQuote()->getGrandTotal());
				$sub_total = $request->getBaseSubtotalInclTax();
       ...
       if ($request->getAllItems()) {
           ...
           foreach ($request->getAllItems() as $item) {
            ...
            if(!empty($products['shipping_cost']) && isset($products['shipping_cost'])){
				$cart_total = $_product->getSpecialPrice();
				$subtotal = Mage::getSingleton('checkout/session')->getQuote()->getSubtotal();
				//custom ship charge
				$custom_ship_charge = $cart_total*.15;
				//echo $special_price*$productOrderQty;
               ...
               if ($shippingQtyBased == 1) {
                 ...
                 // var_dump($sub_total);
                   $shippingPriceProduct = $custom_ship_charge;
               }else{
